...
namespace Telstra.Common
{
    public static class Guard
    {
        ...
        /// <summary>
        /// Throws <see cref="ArgumentException"/> if the given value is negative.
        /// </summary>
        /// <param name="value1"></param>
        /// <param name="value2"></param>
        /// <param name="name1"></param>
        /// <param name="name2"></param>
        public static void NotGreaterThan<T>(T value1, T value2, string name1, string name2) where T : IComparable<T>
        {
            if (value1.CompareTo(value2) > 0)
                throw new ArgumentOutOfRangeException(name1, value1,
                    $"Argument '{name1}' ({value1}) cannot be greater than argument '{name2}' ({value2}).");
        }
        ...
        /// <summary>
        /// Throws an <see cref="ArgumentException"/> if the given value is negative or zero.
        /// </summary>
        /// <param name="value">The value to check.</param>
        /// <param name="name">The name to use when throwing an exception, if necessary.</param>
        public static void NotNegativeOrZero(int value, string name)
        {
            if (value <= 0)
                throw new ArgumentException(name);
        }
        ...
        {
            Guard.NotNullOrEmpty(amount, name);
            if (!int.TryParse(amount, out int convertedAmount))
            {
